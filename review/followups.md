...
- [x] **28. Resolve `Tab` vs `StreamView` materialization.**  Either
...
- [x] **30. `FieldName { Core(CoreField), Extra(String) }`** for
...
- [x] **41. Extract a `LineEditor` test module.**  Drop the
...
- [x] **43. Add a property test for `Filter::matches` being a
...
- [x] **45. Status-line bytes report bytes scanned, not matched.**
      The Phase-1 framing of this item asked whether the summary path
      should narrow its `bytes` reading to filter-matching records;
      discussion clarified the opposite intent — the status line's job
      is to convey the work the engine had to do, so *every* tab's
      "bytes" number should reflect bytes scanned (matching plus
      rejected), matching what the long-op progress bar already
      tracks.  Implemented by switching `format_parse_stats` and
      `SearchOp`'s self-progress diff to read `walked_bytes` instead
      of `bytes`, then deleting the `bytes` field from
      `streamview::ParseStats` entirely once it had no readers.  Two
      unbounded streamview fills (`extend_forward_batch`,
      `extend_backward_batch_n`) didn't accumulate `walked_bytes`
      previously — they now call `stepper.walked_bytes()` like the
      bounded paths do, closing the gap item 23's notes flagged.
      `SummaryOp::finalize` drops its `bytes: self.bytes_read` line
      (and the followup comment pointing here).  Four test fixtures
      that built `ParseStats` literals shed their `bytes:` line.
      Visible effect: under a selective filter the status-line bytes
      number is now bigger (and ticks faster as the engine scans),
      and it matches the progress-bar numerator while a long-op runs.
