...
impl<'a> Stepper<'a> {
    ...
    /// Walks backward up to `n` times and returns the records in
    /// chronological (forward) order.
    ///
    /// Stops early if [`Self::step_backward`] returns `None`, so the
    /// returned vec is at most `n` long.  After this call the
    /// stepper's cursor has retreated by the returned vec's length;
    /// callers that want to resume forward from the *original*
    /// starting position should construct a fresh stepper at the
    /// same cursor (a `Stepper` owns one position that both
    /// directions share, so backward walking is not free).  This is
    /// the primitive `seeit`'s `--before N` uses to assemble the
    /// pre-cursor window.
    pub fn step_backward_n(&mut self, n: usize) -> Vec<MergeRecord> {
...
mod tests {
    ...
    /// Drains a Vec<MergeRecord> into just the event messages,
    /// panicking on any error record.  Symmetric helper to
    /// `forward_msgs`/`backward_msgs` but for a pre-collected vec.
    fn record_msgs(records: Vec<MergeRecord>) -> Vec<String> {
        records
            .into_iter()
            .map(|r| match r.event {
                Ok(e) => e.msg,
    ...
    #[test]
    fn step_backward_n_returns_records_in_chronological_order() {
        // From EOF over a three-record file, walking back 3 yields
        // the records in their original forward order, not the
        // reverse order the underlying stepper would emit.
        let dir = TestDir::new();
        let p = dir.path().join("a.log");
        write_fixture(&p, "x", &[10, 20, 30]);
        let len = std::fs::metadata(&p).unwrap().len();
        ...
        let got = stepper.step_backward_n(3);
        assert_eq!(record_msgs(got), vec!["m10", "m20", "m30"]);
        dir.cleanup();
    }

    #[test]
    fn step_backward_n_clamps_to_available_records() {
        // Asking for more records than exist returns just the ones
        // that do, in chronological order.
        let dir = TestDir::new();
        let p = dir.path().join("a.log");
        write_fixture(&p, "x", &[10, 20, 30]);
        let len = std::fs::metadata(&p).unwrap().len();
        ...
        let got = stepper.step_backward_n(10);
        assert_eq!(record_msgs(got), vec!["m10", "m20", "m30"]);
        dir.cleanup();
    }

    #[test]
    fn step_backward_n_zero_is_empty() {
        let dir = TestDir::new();
        let p = dir.path().join("a.log");
        write_fixture(&p, "x", &[10, 20]);
        let src: Box<dyn Source> = Box::new(FileSource::open(&p).unwrap());
        ...
        assert!(stepper.step_backward_n(0).is_empty());
        // The zero-walk should not have moved the stepper either:
        // a subsequent forward walk still sees both records.
        assert_eq!(forward_msgs(&mut stepper), vec!["m10", "m20"]);
        dir.cleanup();
    }

    #[test]
    fn step_backward_n_at_start_is_empty() {
        // From the default cursor (byte 0 on every source), no
        // records lie behind, so the walk returns empty.
        let dir = TestDir::new();
        let p = dir.path().join("a.log");
        write_fixture(&p, "x", &[10, 20]);
        let src: Box<dyn Source> = Box::new(FileSource::open(&p).unwrap());
        ...
        assert!(stepper.step_backward_n(5).is_empty());
        dir.cleanup();
    }

    #[test]
    fn step_backward_n_from_middle_returns_just_pre_cursor_records() {
        // Position the cursor between the second and third record
        // (just after m20's newline); walking back 5 should return
        // m10 and m20 only — the records strictly before the
        // cursor, in chronological order.
        let dir = TestDir::new();
        let p = dir.path().join("a.log");
        write_fixture(&p, "x", &[10, 20, 30]);
        let src: Box<dyn Source> = Box::new(FileSource::open(&p).unwrap());
        let sources = [src];
        let id = sources[0].id().clone();
        // The cursor sits at the byte where the next forward record
        // (m30) starts.  Discover that position by stepping forward
        ...
        let refs: Vec<&dyn Source> =
            sources.iter().map(|s| s.as_ref()).collect();
        let mut stepper = Stepper::new(refs, Filter::default(), &mid);

        let got = stepper.step_backward_n(5);
        assert_eq!(record_msgs(got), vec!["m10", "m20"]);
        // The mid cursor (untouched in `mid` since `cursor()` is a
        // snapshot) lets a fresh stepper resume forward from m30,
        // which is the pattern seeit uses for `--before N
        // --count M`.
        let refs: Vec<&dyn Source> =
            sources.iter().map(|s| s.as_ref()).collect();
        let mut fwd = Stepper::new(refs, Filter::default(), &mid);
        assert_eq!(forward_msgs(&mut fwd), vec!["m30"]);
