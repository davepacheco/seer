...
/// setting.  `lines.len() >= 1` always: a parse error contributes a
/// single error line; a real event contributes a header plus zero or
/// more extra-field lines.
struct WindowEntry {
    record: MergeRecord,
    lines: Vec<String>,
}
...
impl WindowEntry {
    fn new(record: MergeRecord, show_extras: bool) -> Self {
        let lines = format_record(&record, show_extras);
        Self { record, lines }
    }
...
fn format_record(record: &MergeRecord, show_extras: bool) -> Vec<String> {
    match &record.event {
        Ok(event) => format_event(event, show_extras),
...
impl StreamView {
    ...
    pub fn new(filter: Filter, show_extras: bool) -> Self {
        Self {
            filter,
            show_extras,
    ...
    /// Toggles the `show_extras` setting and reformats every cached
    /// record so the display lines reflect the new setting.  Anchors
    /// onto the same record (clamping the line offset to 0 because
    /// the user's exact line-within-record can't be preserved across
    /// a line-count change).
    pub fn set_show_extras(&mut self, show_extras: bool) {
        if show_extras == self.show_extras {
            return;
        }
        self.show_extras = show_extras;
        for entry in &mut self.records {
            entry.lines = format_record(&entry.record, show_extras);
        }
        ...
        if let Anchor::On { line, .. } = &mut self.anchor {
            *line = 0;
        }
    }
        ...
        for _ in 0..BATCH_SIZE {
            match stepper.step_forward() {
                Some(record) => {
                    bytes += record.length;
                    fetched += 1;
                    self.records.push_back(WindowEntry::new(
                        record,
                        self.show_extras,
                    ));
                }
        ...
        for _ in 0..BATCH_SIZE {
            match stepper.step_backward() {
                Some(record) => {
                    bytes += record.length;
                    fetched += 1;
                    self.records.push_front(WindowEntry::new(
                        record,
                        self.show_extras,
                    ));
                }
...
mod tests {
    ...
    fn set_show_extras_keeps_anchor_on_same_record() {
        ...
        view.scroll_lines(&engine, 1, 20);
        assert_eq!(anchor_msg(&view).as_deref(), Some("m20"));
        view.set_show_extras(true);
        assert_eq!(anchor_msg(&view).as_deref(), Some("m20"));
        view.set_show_extras(false);
        assert_eq!(anchor_msg(&view).as_deref(), Some("m20"));
        dir.cleanup();
    }
